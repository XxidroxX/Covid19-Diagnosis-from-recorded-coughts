...
if __name__ == "__main__":
    ...
    """
    train_transforms = transforms.Compose([#transforms.RandomCrop((32, 32), padding=4),
    ...
    optimizer_ft = optim.SGD(model_ft.parameters(), lr=0.01, momentum=0.9, weight_decay=1e-5)

    # Decay LR by a factor of 0.1 every 7 epochs
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=7, gamma=0.1)

    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=25)
    torch.save(model_ft, "./rete.pth")
    """

    model = torch.load("./rete.pth")
    model.eval()
    path_mp3 = "img/test/"
    glob = '*.jpg'
